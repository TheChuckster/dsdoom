...
int      FAT_putc(int, FAT_FILE *); 
void     FAT_rewind(FAT_FILE *); 
int      FAT_getw(FAT_FILE *); 
int anum;
char ktmp[7][15]; 
FAT_FILE    *FAT_tmpfile(void); 

int		FAT_open(const char *fn, int mode);
void	FAT_close(int hnd);
u32		FAT_read(int hnd, void *buffer, int cnt);
u32		FAT_write(int hnd, const void *buffer, int cnt);
u32		FAT_lseek(int hnd, int offset, int whence); 
/*-----------------------------------------------------------------
ucase
...
int      FAT_getw(FAT_FILE *fp)
{
  int result;
  if (FAT_fread((char*)&result, sizeof(result), 1, fp) != 1)
    return EOF;
  return result;
}
...
int FAT_getc(FAT_FILE * file) {
	return FAT_fgetc(file);
} 
...
FAT_FILE    *FAT_tmpfile(void)
{
	...
	sprintf(ktmp[anum],"/tmpfile%d",anum);
	fp=FAT_fopen(ktmp[anum],"wb");
	anum++;
	return fp;
...
int      FAT_fprintf(FAT_FILE *kf, const char *fmt, ...)
{		
		char line[256];
		int i;
		va_list args; 
		va_start(args, fmt); 
		i=vsnprintf (line,255, fmt, args);
		va_end(args);
		if (i==-1)i=255;	
		FAT_fwrite(line, i, 1, kf);
		return i;
}
#else
int      FAT_fprintf(FAT_FILE *kf, const char *fmt, ...)
...
bool firstdirent=false;
u32 theoldWorkDirCluster;
FAT_DIR * FAT_opendir(char const* dirname) {
	FAT_DIR * newd;

	// Store current working directory
	theoldWorkDirCluster = curWorkDirCluster;
	if (FAT_chdir(dirname) == false) 
	{
		curWorkDirCluster = theoldWorkDirCluster;
		return false; // Couldn't change directory
	}
	// Ok, got it. Alloc a struct to return.
	newd = malloc(sizeof(FAT_DIR));
	if (!newd) {
		errno = ENOMEM;
		return NULL;
	}
	memset(&newd->d_ent, 0, sizeof(struct FAT_dirent));
	firstdirent=true;
	return newd;
} 

struct FAT_dirent * FAT_readdir(FAT_DIR * dir) {
	char* filename[255];
	FILE_TYPE temp_filetype;
	...
	if (!firstdirent){
		temp_filetype= FAT_FindNextFileLFN((char *)filename);
		//printf("next = %s\n",filename);
	...
	{
		firstdirent=false;
		temp_filetype= FAT_FindFirstFileLFN((char *)filename);
		//printf("first = %s\n",filename);
	}
	...
	if (temp_filetype == FT_NONE)
		return NULL;

	if (temp_filetype == FT_DIR)
...
int FAT_open(const char *fn, int mode)
{
		...
		FAT_FILE *fp=FAT_fopen(fn,"r+");
		if (fp == -1) 
		{
			printf("open bad handle\n"); 
			return -1;
		}
...
u32		FAT_read(int hnd, void *buffer, int cnt){
	if (hnd == -1) {printf("read bad handle\n"); return -1;}
	//char* data = (char*)buffer;
	u32 act=FAT_fread(buffer,1,cnt,hnd) ;
	//printf("read %d\n",act);
	return act ;
}
u32		FAT_write(int hnd, const void *buffer, int cnt){
	if (hnd == -1) {printf("write bad handle\n"); return -1;}
	u32 act=FAT_fwrite(buffer,1,cnt,hnd) ;
	return act ;
}
...
u32 FAT_lseek(int hnd, int offset, int whence)
{
	...
	if (hnd == -1)
	{
		printf("seek bad handle\n");
		return -1;
	}
	if (FAT_fseek(hnd,offset,whence ) == -1)
	{
		printf("fseek bad handle %d\n",hnd);
		return -1;
	}

	u32 fpos = FAT_ftell(hnd);

	//printf("hnd=%d,offset=%d whence=%d fpos=%d\n",hnd,offset,whence,fpos);
	return fpos;
}

int FAT_filelength(int hnd){
	int size=FAT_lseek(hnd,0,SEEK_END);
	FAT_fseek(hnd, 0, SEEK_SET);
	return size;
}
